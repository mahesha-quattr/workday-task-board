...
import { format, parseISO, isBefore, addDays, addHours, isToday, isTomorrow, differenceInHours } from "date-fns";
import { motion, AnimatePresence } from "framer-motion";
import { Plus, Bot, Clock, Timer as TimerIcon, Pause, X, ChevronRight, ChevronLeft, Trash2, Play, Flame, AlertTriangle, GripVertical } from "lucide-react";
...
function scoreToBucket(score){
  if (score >= 80) return "P0"; if (score >= 60) return "P1"; if (score >= 40) return "P2"; return "P3";
}
...
function priorityScore({impact=0, urgency=0, effort=0, dueAt=null, hasUnblockedDeps=false, meetingContext=false}){
  let score = (2*impact + 1.5*urgency) - effort;
  if (dueAt) {
    const now = new Date();
    const d = new Date(dueAt);
    const hrs = differenceInHours(d, now);
    if (hrs <= 24) score += 2; else if (hrs <= 72) score += 1;
  }
...
const seedTasks = () => {
  const now = new Date();
  return [
    { id: uid(), title: "Fix login bug for Alpha", project: "alpha", status: "in_progress", impact:4, urgency:5, effort:2, dueAt: addHours(now, 20).toISOString(), ownerType:"self", tags:["bug","auth"], dependencies:[], expectedBy:null },
    { id: uid(), title: "Delegate test data generation to AI", project: "alpha", status: "waiting_ai", impact:3, urgency:3, effort:1, dueAt: addHours(now, 36).toISOString(), ownerType:"ai", tags:["agent"], dependencies:[], expectedBy: addHours(now, 8).toISOString() },
    { id: uid(), title: "Prep for requirements call", project: "beta", status: "ready", impact:3, urgency:4, effort:1, dueAt: addHours(now, 4).toISOString(), ownerType:"self", tags:["meeting"], dependencies:[] },
    { id: uid(), title: "Refactor payment webhook", project: "gamma", status: "blocked", impact:4, urgency:2, effort:3, dueAt: null, ownerType:"self", tags:["tech-debt"], dependencies:[] },
...
const useStore = create((set, get) => ({
  ...
  toggleSelected(id){ set(s => ({ selectedIds: toggleId(s.selectedIds, id) })); },
  clearSelection(){ set({ selectedIds: [] }); },
  deleteSelected(){ set(s => ({ tasks: filterOutByIds(s.tasks, s.selectedIds), selectedIds: [] })); get().persist(); },

  tasks: /** @type{Task[]} */([]),
  filters: { project:"all", status:"all", owner:"all", q:"" },
  // User prefs
  autoReturnOnStop: false,
  setAutoReturnOnStop(v){ set({ autoReturnOnStop: v }); },
  ...
  persist(){
    try {
      ...
      if (typeof localStorage !== 'undefined') localStorage.setItem(STORAGE_KEY, JSON.stringify({ tasks, autoReturnOnStop }));
    } catch (e) { /* ignore storage errors */ }
  },
  addTask(partial){
  ...
  updateTask(id, patch){
    set(s => ({ tasks: s.tasks.map(t => t.id===id ? finalizeTask({...t, ...patch, updatedAt:new Date().toISOString()}) : t) }));
    get().persist();
  },
  deleteTask(id){ set(s => ({ tasks: s.tasks.filter(t => t.id!==id) })); get().persist(); },
  moveTask(id, status){ set(s => ({ tasks: s.tasks.map(t => t.id===id ? {...t, status} : t) })); get().persist(); },
  ...
  stopTimer(id){
    set(s => ({ tasks: s.tasks.map(t => {
      ...
      return { ...t, timeLogSecs: (t.timeLogSecs||0)+add, timerStartedAt: null, status: newStatus };
    }) }));
    get().persist();
  },
  ...
  setDragHoverStatus(status){ set({ dragHoverStatus: status }); },
  clearDrag(){ set({ draggingId: null, dragHoverStatus: null }); },
}));
...
function parseQuickAdd(input){
  ...
  let titleParts = [];
  let project, dueAt = null, ownerType="self", tags=[], impact=undefined, urgency=undefined, effort=undefined, priorityBucket=null, expectedBy=null;
  for (let i=0;i<tokens.length;i++){
    const raw = tokens[i].replaceAll('"','');
    if (raw.startsWith('#')) { project = raw.slice(1); continue; }
    if (raw.startsWith('+')) { tags.push(raw.slice(1)); continue; }
    if (/^!p[0-3]$/i.test(raw)) { priorityBucket = raw.toUpperCase().slice(1); continue; }
    if (raw === "@ai") { ownerType = "ai"; continue; }
    if (raw === "@me") { ownerType = "self"; continue; }
    if (raw.startsWith("impact:")) { impact = Number(raw.split(":")[1])||undefined; continue; }
    if (raw.startsWith("urgency:")) { urgency = Number(raw.split(":")[1])||undefined; continue; }
    if (raw.startsWith("effort:")) { effort = Number(raw.split(":")[1])||undefined; continue; }
    ...
    if (/^\d{1,2}:\d{2}$/.test(raw) && dueAt===null && tokens[i-1] && tokens[i-1].startsWith("due:")){
      const last = tokens[i-1].slice(4);
      dueAt = parseDueToken([last, raw]);
      continue;
    }
    if (raw.startsWith("expect:")) { // expected AI callback by time
...
function Badge({children, className}){ return <span className={clsx("inline-flex items-center rounded-full border px-2 py-0.5 text-xs", className)}>{children}</span>; }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="font-semibold text-slate-800 dark:text-slate-100">{STATUS_META[status].label}</h3>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <Badge className="bg-slate-100 text-slate-700 border-slate-300">
              <TimerIcon className="w-3.5 h-3.5 mr-1"/>{elapsedLabel}{isRunning?" • running":""}
            </Badge>
          </div>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
            {task.project && <span className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">#{task.project}</span>}
            {task.tags?.map(t => <span key={t} className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">+{t}</span>)}
        ...
        <div className="flex items-center gap-1">
          <button title="Move left" onClick={()=>{
            ...
            if (idx > 0) useStore.getState().moveTask(task.id, /** @type{Status} */(order[idx-1]));
          }} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><ChevronLeft className="w-4 h-4"/></button>
          {isRunning ? (
            <button title="Pause" onClick={()=>stopTimer(task.id)} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><Pause className="w-4 h-4"/></button>
          ) : (
            <button title="Start focus timer" onClick={()=>startTimer(task.id)} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><Play className="w-4 h-4"/></button>
          )}
          <button title="Move right" onClick={()=>{
            const order = /** @type{Status[]} */(Object.keys(STATUS_META));
            const idx = order.indexOf(task.status);
            if (idx < order.length-1) useStore.getState().moveTask(task.id, /** @type{Status} */(order[idx+1]));
          }} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><ChevronRight className="w-4 h-4"/></button>
        </div>
      </div>
...
function TaskDrawer({ task, onClose }){
  ...
  function save(patch){ update(task.id, patch); }

  const scoreMath = useMemo(()=>{
    const s = priorityScore({impact: local.impact, urgency: local.urgency, effort: local.effort, dueAt: local.dueAt});
  ...
  return (
    <motion.div initial={{opacity:0, x:40}} animate={{opacity:1, x:0}} exit={{opacity:0, x:40}}
      ...
      <div className="mt-2 space-y-2">
        <Field label="Title">
          <input value={local.title} onChange={e=>setLocal({...local, title:e.target.value})} onBlur={()=>save({title:local.title})}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
        </Field>
        <Field label="Project">
          <input value={local.project||""} placeholder="#project" onChange={e=>setLocal({...local, project:e.target.value})} onBlur={()=>save({project:local.project})}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
        </Field>
        <Field label="Status">
          <select value={local.status} onChange={e=>{ const v=/** @type{Status} */(e.target.value); setLocal({...local, status:v}); save({status:v}); }}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900">
            {Object.keys(STATUS_META).map(k=> <option key={k} value={k}>{STATUS_META[k].label}</option>)}
          </select>
        </Field>
        <Field label="Owner">
          <select value={local.ownerType} onChange={e=>{ const v=/** @type{OwnerType} */(e.target.value); setLocal({...local, ownerType:v}); save({ownerType:v}); }}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900">
        ...
        {local.ownerType==='ai' && (
          <Field label="Expected by">
            <input type="datetime-local" value={local.expectedBy? new Date(local.expectedBy).toISOString().slice(0,16):""}
              onChange={e=>{ const iso = e.target.value? new Date(e.target.value).toISOString(): null; setLocal({...local, expectedBy: iso}); save({expectedBy: iso}); }}
              className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
          </Field>
        )}
        <Field label="Due">
          <input type="datetime-local" value={local.dueAt? new Date(local.dueAt).toISOString().slice(0,16):""}
            onChange={e=>{ const iso = e.target.value? new Date(e.target.value).toISOString(): null; setLocal({...local, dueAt: iso}); save({dueAt: iso}); }}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
        </Field>
        <Field label="Impact / Urgency / Effort">
          <div className="flex items-center gap-3">
            <NumberInput value={local.impact} onChange={v=>{ setLocal({...local, impact:v}); save({impact:v}); }} />
            <NumberInput value={local.urgency} onChange={v=>{ setLocal({...local, urgency:v}); save({urgency:v}); }} />
            <NumberInput value={local.effort} onChange={v=>{ setLocal({...local, effort:v}); save({effort:v}); }} />
          </div>
        </Field>
        <Field label="Priority">
          <div className="flex items-center gap-2">
            <Badge className={PRIORITY_COLORS[scoreMath.b]}>{scoreMath.b}</Badge>
            <button onClick={()=>{ const s = priorityScore({impact: local.impact, urgency: local.urgency, effort: local.effort, dueAt: local.dueAt}); const b = scoreToBucket(s); setLocal({...local, priorityBucket: b}); save({ score:s, priorityBucket: b }); }}
              className="px-2 py-1 text-sm rounded border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">Rescore</button>
            <span className="text-xs text-slate-500">score = (2×impact + 1.5×urgency) − effort {local.dueAt?" + due boost":""}</span>
          </div>
        </Field>
        <Field label="Tags">
          <input value={local.tags?.join(" ")||""} placeholder="+tag +another" onChange={e=>{ const arr=e.target.value.split(/\s+/).filter(Boolean).map(s=>s.replace(/^\+/,'')); setLocal({...local, tags:arr}); save({tags:arr}); }}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
        </Field>
        <Field label="Notes">
          <textarea rows={6} value={local.description||""} onChange={e=>setLocal({...local, description:e.target.value})} onBlur={()=>save({description:local.description})}
            className="w-full px-3 py-2 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
        </Field>
        <div className="flex items-center justify-between pt-2">
          {task.status==='waiting_ai' && (
            <button onClick={()=>useStore.getState().updateTask(task.id, { status: 'ready', ownerType: 'self', expectedBy: null })} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700"><Bot className="w-4 h-4"/> Simulate AI Update</button>
          )}
          <div className="flex-1"/>
          <button onClick={()=>{ del(task.id); onClose(); }} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-rose-600 text-white hover:bg-rose-700"><Trash2 className="w-4 h-4"/> Delete</button>
        </div>
      </div>
    </motion.div>
  );
...
function Toolbar(){
  ...
  const onAdd = () => {
    ...
    const base = { title: p.title || "Untitled", project: p.project, status: p.ownerType==='ai' ? 'waiting_ai':'ready', ownerType: p.ownerType, tags: p.tags, dueAt: p.dueAt, expectedBy: p.expectedBy };
  ...
  return (
    <div className="sticky top-0 z-10 -mx-1 mb-3 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-slate-900/60 p-2 rounded-xl border border-slate-200 dark:border-slate-700">
      <div className="flex flex-col lg:flex-row gap-2 items-stretch lg:items-center">
        ...
        <div className="flex items-center gap-2">
          <select value={filters.project} onChange={e=>setFilters({project:e.target.value})} className="px-2 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900">
            <option value="all">All projects</option>
            <option value="alpha">alpha</option>
            <option value="beta">beta</option>
            <option value="gamma">gamma</option>
          </select>
          <select value={filters.owner} onChange={e=>setFilters({owner:e.target.value})} className="px-2 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900">
            <option value="all">Any owner</option>
            <option value="self">Me</option>
            <option value="ai">AI</option>
            <option value="other">Other</option>
          </select>
          <input value={filters.q} onChange={e=>setFilters({q:e.target.value})} placeholder="Filter text" className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"/>
          {/* Pref: auto return */}
          <label className="ml-2 inline-flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300 select-none">
            <input type="checkbox" checked={autoReturnOnStop} onChange={e=>setAutoReturnOnStop(e.target.checked)} />
      ...
      {selectedIds.length>0 && (
        <div className="mt-2 flex items-center justify-between rounded-xl border border-rose-300 bg-rose-50 text-rose-900 px-3 py-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={onBulkDelete} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-rose-600 text-white hover:bg-rose-700"><Trash2 className="w-4 h-4"/>Delete selected</button>
...
function Board(){
  ...
  const filtered = useMemo(()=>{
    return tasks.filter(t => {
      ...
      if (filters.q && !(`${t.title} ${t.description||''} ${t.tags.join(' ')}`.toLowerCase().includes(filters.q.toLowerCase()))) return false;
      return true;
    }).sort((a,b)=>{
      // Sort by status lane then priority score desc then due date asc
      if (a.status !== b.status) return Object.keys(STATUS_META).indexOf(a.status) - Object.keys(STATUS_META).indexOf(b.status);
  ...
  const grouped = useMemo(()=>{
    ...
    const g = {backlog:[], ready:[], in_progress:[], waiting_ai:[], waiting_other:[], blocked:[], in_review:[], done:[]};
...
function runSelfTests(){
  const results = [];
  const test = (name, fn) => { try { const r = fn(); results.push({name, ok: r===true}); } catch { results.push({name, ok:false}); } };

  // Test: parseQuickAdd basics
  test('parseQuickAdd tokens', () => {
    const p = parseQuickAdd('Fix login #alpha !p0 due:today 17:00 @ai +auth impact:4 urgency:5 effort:2 expect:today');
    return (
      p.project==='alpha' && p.ownerType==='ai' && p.tags.includes('auth') && !!p.dueAt && !!p.expectedBy &&
      p.impact===4 && p.urgency===5 && p.effort===2
    );
  });
  ...
  test('scoreToBucket boundaries', () => {
    return scoreToBucket(81)==='P0' && scoreToBucket(80)==='P0' && scoreToBucket(79)==='P1' &&
           scoreToBucket(60)==='P1' && scoreToBucket(59)==='P2' && scoreToBucket(40)==='P2' &&
           scoreToBucket(39)==='P3';
  });
  ...
  test('computeElapsedSecs adds running time', () => {
    const now = new Date();
    const t = { timeLogSecs: 30, timerStartedAt: new Date(now.getTime()-90*1000).toISOString() };
    const secs = computeElapsedSecs(t, now);
    return secs >= 119 && secs <= 121; // allow 1s jitter
  });
  test('formatDurationShort formatting', () => {
    return formatDurationShort(3670) === '1h 1m' && formatDurationShort(125) === '2m 5s' && formatDurationShort(9) === '9s';
  });
  ...
  test('toggleId adds/removes id', () => {
    ...
    return a.includes('x') && b.includes('x') && b.includes('y') && !c.includes('x') && c.includes('y');
  });
...
function SelfTestResults(){
  ...
  return (
    <div className={clsx("mt-6 text-xs", ok===total?"text-slate-500":"text-rose-600")}>Self-tests: {ok}/{total} passed.</div>
  );
}
...
export default function WorkdayTaskBoardApp(){
  ...
  useEffect(()=>{ init(); }, [init]);
  useEffect(()=>{ const id = setInterval(()=>persist(), 1000); return ()=>clearInterval(id); }, [persist]);
      ...
      <div className="max-w-7xl mx-auto px-3 py-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Keyboard-first board for tasks, meetings, bugs, and AI handoffs.</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={()=>setDark(v=>!v)} className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800">{dark?"Light":"Dark"} mode</button>
          </div>
        </div>
        ...
        <SelfTestResults/>
        <div className="mt-2 text-xs text-slate-500">
          Drag to any column (left or right). Time chips show running focus time; toggle &quot;Return to Ready on pause&quot; in the toolbar.
        </div>
      </div>
    </div>
  );
